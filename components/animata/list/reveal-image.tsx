...
function RevealImageListItem({ text, images }: ShowImageListItemProps) {
  ...
  return (
    <div className="group relative h-fit w-full overflow-visible py-8">
      ...
      <h1 className="text-7xl font-black text-foreground transition-all duration-500 group-hover:opacity-40 text-center">
        {" "}
        {/* Added text-center */}
        {text}
      </h1>
...
export default function RevealImageList() {
  ...
  return (
    <div className="flex flex-col items-center gap-1 rounded-sm bg-background px-8 py-4 w-full">
      {" "}
      {/* Added items-center and w-full */}
      <h3 className="text-sm font-black uppercase text-muted-foreground text-center w-full">
        {" "}
        {/* Added text-center and w-full */}
        We are here.
      </h3>
      {items.map((item, index) => (
        <RevealImageListItem
